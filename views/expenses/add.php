...
$debug_categories = $conn->query("SELECT COUNT(*) as count FROM categories WHERE user_id = $user_id AND type = 'expense'")->fetch_assoc();
$debug_wallets = $conn->query("SELECT COUNT(*) as count FROM wallets WHERE user_id = $user_id")->fetch_assoc();

if ($debug_categories['count'] == 0) {
    $warning_message = "You need to create expense categories first. <a href='../categories.php' class='underline'>Create categories here</a>";
}
    ...
    if ($amount > 0 && !empty($category_id) && !empty($wallet_id) && !empty($transaction_date)) {
        ...
        if ($wallet && $wallet['balance'] >= $amount) {
            ...
            try {
                ...
                $stmt_insert->bind_param("iiidss", $user_id, $wallet_id, $category_id, $amount, $description, $transaction_date);
                $stmt_insert->execute();
                ...
                header("Location: " . $base_url . "/views/expenses/manage_expenses.php");
                exit();
