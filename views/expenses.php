...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($amount > 0 && !empty($category_id) && !empty($account_id) && !empty($transaction_date)) {
        ...
        try {
            $stmt = $conn->prepare("INSERT INTO transactions (user_id, account_id, category_id, type, amount, description, transaction_date) VALUES (?, ?, ?, 'expense', ?, ?, ?)");
            ...
            $_SESSION['success_message'] = "Pengeluaran untuk '" . htmlspecialchars($description) . "' berhasil dicatat!";
...
$categories_result = $conn->query("SELECT id, name FROM categories WHERE type = 'expense'");
        ...
        <form action="expenses.php" method="POST">
             <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                 ...
                 <div>
                    <label for="account_id" class="block text-gray-700 font-semibold mb-2">Sumber Dana (Akun)</label>
